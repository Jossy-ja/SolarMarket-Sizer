...
# Rough informational peak estimate (actual peak depends on which loads
# overlap in generate_year_profile — see load_df["total_load_kw"].max()
# after generation for the real figure).
approx_peak_kw = (
...
print(f"Approximate peak load (informational only): {approx_peak_kw:.1f} kW")
""")
...
print("Total annual demand (kWh):", round(load_df["total_load_kw"].sum(), 0))
print("Actual peak load (kW):", round(load_df["total_load_kw"].max(), 1))
load_df["total_load_kw"].loc["2025-01-06":"2025-01-12"].plot(
