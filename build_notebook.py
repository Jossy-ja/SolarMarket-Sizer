...
scratch. Note: the notebook now includes a demand-scenario sensitivity
section (Section 8) that was added by hand after the first version of this
script was written — this script has been kept in sync with that, so
running it regenerates the full notebook including that section rather
than reverting to an older, simpler version. If you extend the notebook
further, update this script to match, or the two will drift apart again.
"""
...
1. Define an estimated hourly load profile for the market (base + low/high demand scenarios)
...
6. Stress-test the recommendation against demand-scenario uncertainty
...
from src.load_profile import generate_year_profile, apply_demand_scenario
...
# These map directly onto load_profile.generate_year_profile()'s
# parameters, so any run of this notebook is traceable to the exact
# end-use assumptions that produced it.
NUM_STALLS = 80
STALL_LOAD_KW = 0.35        # lighting, small fridges, phone charging, fans (per stall)
LIGHTING_KW = 5.0           # common-area lighting during operating hours
REFRIGERATION_KW = 3.0      # assumed to run continuously
WATER_PUMP_KW = 2.0         # during operating hours
SECURITY_KW = 1.0           # assumed to run continuously
OTHER_FIXED_KW = 2.0        # other common-area loads during operating hours

OPEN_HOUR = 6
CLOSE_HOUR = 19
MARKET_DAYS = [0, 1, 2, 3, 4, 5]   # closed Sundays (0=Mon ... 6=Sun)

# Demand scenarios, used throughout the notebook (Section 2 summary,
# Section 8 sweep) to stress-test the sizing recommendation against
# demand uncertainty rather than treating the base estimate as exact.
DEMAND_SCENARIOS = {
    "low": 0.80,
    "base": 1.00,
    "high": 1.20,
...
The market demand profile is synthetic and is constructed from explicit
end-use assumptions rather than metered electricity data. Three demand
scenarios are retained for later sensitivity analysis:
...
# The dispatch/sweep functions expect a column called load_kw
...
The first sweep below uses the **base demand scenario**. The low/base/high
demand scenarios are retained in `load_profiles` and propagate into the
multi-scenario sizing analysis in Section 8.
...
md("""\
## 8. Demand-scenario sensitivity analysis

The sections above use the **base** demand scenario only. This section
repeats the full PV x battery sweep for the **low** (80%) and **high**
(120%) demand scenarios as well, so the sizing recommendation in
Section 7 can be stress-tested against demand uncertainty rather than
treated as fixed.
""")
...
print("Scenario sweep shape:", scenario_sweep_df.shape,
      "(expected:", len(PV_SIZES_KWP) * len(BATTERY_SIZES_KWH) * 3, "rows)")

print("\\nRows per scenario:")
print(scenario_sweep_df["demand_scenario"].value_counts())

print("\\nScenario summary (mean across PV/battery sizes):")
print(
    scenario_sweep_df.groupby("demand_scenario")[
        ["total_load_kwh", "renewable_fraction", "grid_dependency", "curtailment_fraction"]
    ].mean()
)

n_dupes = scenario_sweep_df.duplicated(
    subset=["demand_scenario", "pv_size_kwp", "battery_size_kwh"]
).sum()
print(f"\\nDuplicate (scenario, pv_size, battery_size) combinations: {n_dupes}")

scenario_sweep_df.to_csv("../outputs/scenario_sensitivity_sweep_results.csv", index=False)
""")
...
- How sensitive the recommendation is to demand uncertainty, per the
  Section 8 low/base/high scenario sweep
